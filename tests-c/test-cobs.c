...
void test_cobs_fixed(void)
{
    ...
    for (i = 0; i < dimof(cobs_tests); i++)
    {
        ...
        TEST_ASSERT_EQUAL_size_t(cobs_tests[i].encoded_len, encode_result.out_len);
        TEST_ASSERT_EQUAL_UINT8_ARRAY(cobs_tests[i].encoded_ptr, encode_out, cobs_tests[i].encoded_len);

        decode_result = cobs_decode(decode_out, sizeof(decode_out), encode_out, encode_result.out_len);
        TEST_ASSERT_EQUAL_size_t(cobs_tests[i].data_len, decode_result.out_len);
        if (cobs_tests[i].data_len)
        {
            TEST_ASSERT_EQUAL_UINT8_ARRAY(cobs_tests[i].data_ptr, decode_out, cobs_tests[i].data_len);
        }
    }
}
